...
class CurationScreen(MDScreen):
    ...
    def on_language_change(self):
        ...
        self.ids.found_examples_label.text = self.app.translator.t(
            'found_examples')
        self.ids.add_selected_button.text = self.app.translator.t(
            'add_selected_button')
    ...
    def populate_screen(self):
        ...
        for ex in self.enriched_data.get('examples', []):
            ...
            highlighted_original = tagged_original.replace('<b>',
                                                           '[b]').replace(
                                                               '</b>', '[/b]')
            highlighted_translation = tagged_translation.replace(
                '<b>', '[b]').replace('</b>', '[/b]')
            ...
            item.add_widget(
                IconRightWidget(
                    icon="delete-outline",
                    on_release=lambda x, i=item: self.delete_example(i)))
    ...
    def save_curated_items(self):
        ...
        if not items:
            s = Snackbar()
            s.text = "Нет примеров для сохранения!"
            s.open()
            return
        s = Snackbar()
        s.text = "Сохранение... Это может занять несколько секунд."
        s.open()
        save_data = [{
            'original': i._original_phrase,
        ...
        thread = Thread(target=self._blocking_save,
                        args=(save_data, image_path))
        thread.start()
    ...
    async def _async_save_items(self, phrases_data, image_path):
        ...
        tasks = [
            self.process_and_save_item(p, image_path, db_manager)
    ...
    async def process_and_save_item(self, phrase_info, image_path, db_manager):
        original_phrase, translation = phrase_info['original'], phrase_info[
            'translation']
        audio_path = await generate_audio(original_phrase, self.lang_code,
                                          "example")
