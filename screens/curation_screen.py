# Файл: screens/curation_screen.py (ФИНАЛЬНАЯ ВЕРСИЯ ЭТАПА)
import asyncio
...
class CurationScreen(MDScreen):
    ...
    def populate_screen(self):
        ...
        for ex in self.enriched_data.get('examples', []):
            ...
            # --- ИЗМЕНЕНИЕ: Превращаем <b> в [b] для Kivy ---
            highlighted_kivy = tagged_original.replace('<b>', '[b]').replace('</b>', '[/b]')
            
            item = TwoLineAvatarIconListItem(
                text=highlighted_kivy, 
                secondary_text=translation
            )
            
            # Сохраняем чистую фразу, удалив теги
    ...
    # ... (остальные методы save_..., delete_... и т.д. остаются без изменений)
