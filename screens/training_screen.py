...
class TrainingScreen(MDScreen):
    ...
    def on_language_change(self):
        ...
        self.ids.answer_input.hint_text = self.app.translator.t(
            'your_answer_hint')

        if self._current_mode == 'show_answer':
            self.ids.action_button.text = self.app.translator.t(
                'show_answer_button')
        else:
            self.ids.action_button.text = self.app.translator.t(
                'check_answer_button')
    ...
    def show_next_card(self):
        ...
        if self._session_total > 0:
            progress = (self._session_total -
                        len(self.all_cards)) / self._session_total * 100
            self.ids.progress_bar.value = progress

    def _setup_card_ui(self, data):
        self.ids.question_label.text = data.get('text', '')
        image_path = data.get('image')
        self.ids.card_image.source = image_path if image_path else 'assets/tmp/placeholder.png'
    ...
    def check_typed_answer(self):
        ...
        if user_answer.lower() == correct_answer.lower():
            ...
            self.ids.correct_answer_label.text = self.app.translator.t(
                'correct_answer_is', answer=correct_answer)
    ...
    def evaluate_answer(self, quality: str):
        ...
        srs_result = calculate_next_due_date(repetitions=card['repetitions'],
    ...
    def end_training(self):
        ...
        Clock.schedule_once(
            lambda dt: setattr(self.manager, 'current', 'deck_list'), 2)
