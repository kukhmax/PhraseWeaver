# Файл: core/enrichment.py (ФИНАЛЬНАЯ ВЕРСИЯ)
...
# ... (logging, DIRS, ensure_dir_exists - без изменений)
...
# --- ИЗМЕНЕНИЕ: Добавляем поддержку европейского португальского ---
async def generate_audio(text: str, lang: str, prefix: str):
    try:
        ...
        # Определяем Top-Level Domain для gTTS
        tld_map = {'pt': 'pt'} # Для португальского используем домен .pt
        tld = tld_map.get(lang, 'com') # Для всех остальных - стандартный .com
...
async def enrich_phrase(keyword: str, full_sentence: str, lang_code: str, target_lang: str = 'ru') -> dict | None:
    # ... (без изменений)
    logging.info(f"--- НАЧАЛО ОБОГАЩЕНИЯ (v5) для '{keyword}' ---")
    ...
    tasks = [get_translation(keyword, from_lang=lang_code, to_lang=target_lang),
             download_and_save_image(image_url_from_api, english_image_query),
             generate_audio(keyword, lang_code, "keyword")]
    user_sentence_to_translate = full_sentence if full_sentence and full_sentence != keyword else None
    if user_sentence_to_translate:
        tasks.append(get_translation(user_sentence_to_translate, from_lang=lang_code, to_lang=target_lang))
    gathered_results = await asyncio.gather(*tasks)
    keyword_translation, image_path, keyword_audio_path = gathered_results[0], gathered_results[1], gathered_results[2]
    full_sentence_translation = gathered_results[3] if user_sentence_to_translate else None
