...
def ensure_dir_exists(*dirs):
    [d.mkdir(parents=True, exist_ok=True) for d in dirs if not d.exists()]

ensure_dir_exists(AUDIO_DIR, IMAGE_DIR)

async def get_translation(text: str, from_lang: str,
                          to_lang: str) -> str | None:
    try:
...
async def generate_audio(text: str, lang: str, prefix: str):
    try:
        ...
        tts = await asyncio.get_running_loop().run_in_executor(
            None, lambda: gTTS(text=text, lang=lang, tld=tld, slow=False))
        await asyncio.get_running_loop().run_in_executor(
            None, tts.save, str(path))
...
async def enrich_phrase(keyword: str, full_sentence: str, lang_code: str,
                        target_lang: str) -> dict | None:
    logging.info(
        f"--- НАЧАЛО ОБОГАЩЕНИЯ (v6) для '{keyword}' на '{target_lang}' ---")
    lang_map = {
        'en': 'English',
        'ru': 'Russian',
        'es': 'Spanish',
    ...
    ai_data = await generate_examples_with_ai(
        keyword, lang_map.get(lang_code, lang_code), target_lang_full)
    if not ai_data: return None

    image_query, examples = ai_data.get("image_query",
                                        keyword), ai_data.get("examples", [])

    english_image_query = image_query
    if lang_code != 'en':
        translated_query = await get_translation(image_query,
                                                 from_lang=lang_code,
                                                 to_lang='en')
    ...
    tasks = [
        ...
        generate_audio(keyword, lang_code,
                       "keyword")  # <-- Вызов теперь снова правильный
    ]
    if full_sentence and full_sentence != keyword:
        tasks.append(
            get_translation(full_sentence,
                            from_lang=lang_code,
                            to_lang=target_lang))

    gathered_results = await asyncio.gather(*tasks)

    keyword_translation, image_path, keyword_audio_path = gathered_results[
        0], gathered_results[1], gathered_results[2]
    full_sentence_translation = gathered_results[3] if (
        full_sentence and full_sentence != keyword) else None

    return {
        'keyword': keyword,
        'translation': keyword_translation,
        'full_sentence_translation': full_sentence_translation,
        'examples': examples,
